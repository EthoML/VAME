...
Now you can run the pipeline. At this point, you should pass the names of the pose estimation keypoints to be used for egocentric alignment. You can also pass other preprocessing and training set arguments.
...
trainingset_kwargs = {"test_fraction": 0.1}

pipeline.run_pipeline(
    preprocessing_kwargs=preprocessing_kwargs,
    trainingset_kwargs=trainingset_kwargs,
)
```
...
```python
pipeline.visualize_hierarchical_tree(segmentation_algorithm="hmm")
```

```python
pipeline.visualize_umap(show_figure="plotly")
```
