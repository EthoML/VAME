...
- **video_type** (`Optional[str]`): The extension of the project's raw videos (`.mp4` or `.avi`), recorded when they are loaded. `None` when the project has no videos or mixes formats; each session's video is resolved from `data/raw` either way.
...
- **keypoints** (`List[str]`): Names of keypoints extracted from pose estimation data.
